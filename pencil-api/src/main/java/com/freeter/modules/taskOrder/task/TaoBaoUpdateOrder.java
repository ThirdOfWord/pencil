...
/**
 * @ClassName TaoBaoUpdateOrder
 * @Author WangKui
 * @Date 2020/8/3 22:48
 * @Version 1.0
 **/
@Component
public class TaoBaoUpdateOrder {
    @Autowired
    ...
    */
    ...
    public void getListOrder(){
        EntityWrapper<GoodsOrderEntity> goodsOrderWrapper = new EntityWrapper<>();
        goodsOrderWrapper.in("supplier_id", "3,4");
        goodsOrderWrapper.eq("order_status_old", 12);
        List<GoodsOrderEntity> goodsOrderList = goodsOrderService.selectList(goodsOrderWrapper);
        getTime(goodsOrderList);
    }
    public void getTime(List<GoodsOrderEntity> goodsOrderList){
        if (!goodsOrderList.isEmpty()) {
            ...
            for(int i=size-1;i>=0;i--){
                if (startTime == 0) {
                    endTime = goodsOrderList.get(i).getOrdertime();
                    startTime = endTime - 60 * 18;
                    endTime = endTime+5;
                    goodsOrderList.remove(i);
                } else {
                    int ordertime = goodsOrderList.get(i).getOrdertime();//订单创建时间
            ...
            //根据筛选获取开始时间和结束时间
            try {
                testScheduled(startTime,endTime);
            } catch (Exception E) {
                E.printStackTrace();
            }
            if (!goodsOrderList.isEmpty()) {
    ...
    public void testScheduled(int startTime,int endTime) throws Exception{
        TbkOrderDetailsGetRequest req = new TbkOrderDetailsGetRequest();
        long pageNo=1;
        req.setPageSize(20L);
        req.setStartTime(DateUtil.timeStampToStr(startTime));
        req.setEndTime(DateUtil.timeStampToStr(endTime));
        ...
        System.out.println("TaoBaoUpdateOrder run:" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss")));
    }
    private void getPageOrder(long pageNo,BigDecimal liRun,TbkOrderDetailsGetRequest req) throws Exception{
